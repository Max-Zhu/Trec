...
import os, sys, re, codecs, json, fileinput
import optparse
import numpy
...
class ldig(object):
    ...
    def detect(self, options, args):
        ...
        log_likely = likelihood(param, labels, trie, inputs, options)
...
def likelihood(param, labels, trie, inputs, options):
    K = len(labels)
    corrects = numpy.zeros(K, dtype=int)
    counts = numpy.zeros(K, dtype=int)
    ...
    for line in inputs:
        ...
        label, text, org_text = normalize_text(line)
